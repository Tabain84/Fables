...
namespace Server.Engines.XmlSpawner2
{
	public class XmlCustomDefenses : XmlAttachment
	{
		...
		new public static void Initialize()
		{
			...
			foreach (var x in XmlAttach.AllAttachments.Values)
		...
		{
			if (defenselist != null)
				foreach (var sid in defenselist)
					AddSpecial(sid);
		}
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			foreach (SpecialDefense s in Specials) writer.Write(s.DefenseID.ToString());
		}
			...
			switch (version)
			{
				case 0:
					...
					for (var i = 0; i < count; i++)
					{
						...
						try
						{
							var stype = (SpecialDefenses)Enum.Parse(typeof(SpecialDefenses), sname);
		...
		public static void AddDefense(object target, SpecialDefenses defense)
		{
			...
			var a = (XmlCustomDefenses)XmlAttach.FindAttachment(target, typeof(XmlCustomDefenses));
		...
		private void FillSpecialsList()
		{
			...
			{
				var s = GetSpecialDefense(sid);
				if (s != null) Specials.Add(s);
			}
		}

		private void SetRandomSpecials(int count)
		{
			var nspecials = AllSpecials.Count;
			...
			if (nspecials > 0)
			{
				...
				while (total < count)
				{
					var rand = Utility.Random(nspecials);
					var nrtries = 0;
					...
					var s = GetSpecialDefense((SpecialDefenses)rand);
					if (s != null)
					{
						Specials.Add(s);
						total++;
		...
		{
			foreach (ActiveCombo c in Combos) c.PositionInSequence = 0;
		}
		...
		public static bool CheckRequirements(Mobile from, SpecialDefense s)
		{
			...
			if (s.Skills != null && s.MinSkillLevel != null)
			{
				...
				for (var i = 0; i < s.Skills.Length; i++)
					...
					{
						var skill = @from.Skills[s.Skills[i]];
			...
			if (s.Reagents != null && s.Quantity != null)
			{
				...
				for (var i = 0; i < s.Reagents.Length; i++)
				{
					// go through each reagent
					var ret = from.Backpack.FindItemByType(s.Reagents[i], true);
		...
		public override int OnArmorHit(Mobile attacker, Mobile defender, Item armor, BaseWeapon weapon, int damageGiven)
		{
			...
			// apply the attack
			var damage = DoSpecialDefense(attacker, defender, weapon, damageGiven, m_SelectedDefense);

			if (m_SelectedDefense.KarmaReq > 0) defender.SendMessage("and lose a little karma.");
		...
		{
			var msg = "Special Defenses:";

			foreach (SpecialDefense s in Specials) msg += String.Format("\n{0}", s.Name);
		...
		public override void OnAttach()
		{
			...
			if (!(AttachedTo is BaseShield)) Delete();
		}
		...
		private class ComboTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				// refresh the gump
				if (m_from != null) m_from.SendGump(new CustomDefenseGump(m_from, m_attachment));
			}
		}
		...
		private class CustomDefenseInfoGump : Gump
		{
			...
			public CustomDefenseInfoGump(Mobile from, XmlCustomDefenses a, SpecialDefense s) : base(0, 0)
			{
				...
				if (s.StrReq > 0) text.AppendFormat("\n     {0} Str", s.StrReq);
				if (s.DexReq > 0) text.AppendFormat("\n     {0} Dex", s.DexReq);
				if (s.IntReq > 0) text.AppendFormat("\n     {0} Int", s.IntReq);
				...
				if (s.ManaReq > 0) text.AppendFormat("\n     {0} Mana", s.ManaReq);
				if (s.StamReq > 0) text.AppendFormat("\n     {0} Stamina", s.StamReq);
				if (s.HitsReq > 0) text.AppendFormat("\n     {0} Hits", s.HitsReq);
				if (s.KarmaReq > 0) text.AppendFormat("\n     {0} Karma", s.KarmaReq);
		...
		private class CustomDefenseGump : Gump
		{
			...
			public CustomDefenseGump(Mobile from, XmlCustomDefenses a) : base(0, 0)
			{
				...
				var specialcount = a.Specials.Count;
				...
				{
					var s = (SpecialDefense)m_attachment.Specials[i];
			...
			public override void OnResponse(NetState state, RelayInfo info)
			{
				...
				for (var i = 0; i < m_attachment.Specials.Count; i++)
				{
					var s = (SpecialDefense)m_attachment.Specials[i];
