...
namespace Server.Engines.XmlSpawner2
{
	public class XmlCustomAttacks : XmlAttachment
	{
			...
			SpecialAttack special)
		{
			...
			switch (special.AttackID)
			{
				...
				case SpecialAttacks.ParalyzingFear:
				{
					...
					// flee
					if (defender is BaseCreature) ((BaseCreature)defender).ForceFleeUntil = DateTime.Now.AddSeconds(6);
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			foreach (SpecialAttack s in Specials) writer.Write(s.AttackID.ToString());
		}
			...
			switch (version)
			{
				case 0:
					...
					for (var i = 0; i < count; i++)
					{
						...
						try
						{
							var stype = (SpecialAttacks)Enum.Parse(typeof(SpecialAttacks), sname);
		...
		public static void AddAttack(object target, SpecialAttacks attack)
		{
			...
			var a = (XmlCustomAttacks)XmlAttach.FindAttachment(target, typeof(XmlCustomAttacks));
		...
		private void SetRandomSpecials(int count)
		{
			...
			if (nspecials > 0)
			{
				...
				while (total < count)
				{
					...
					var s = GetSpecialAttack((SpecialAttacks)rand);
					if (s != null)
					{
						Specials.Add(s);
						total++;
		...
		public override void OnWeaponHit(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven)
		{
			...
			if (m_SelectedAttack.KarmaReq > 0) attacker.SendMessage("and lose a little karma.");
		...
		{
			var msg = "Special Attacks:";

			foreach (SpecialAttack s in Specials) msg += String.Format("\n{0}", s.Name);
		...
		public override void OnAttach()
		{
			...
			if (!(AttachedTo is BaseWeapon)) Delete();
		}
		...
		private class ComboTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				// refresh the gump
				if (m_from != null) m_from.SendGump(new CustomAttacksGump(m_from, m_attachment));
			}
		}
		...
		private class CustomAttacksGump : Gump
		{
			...
			public CustomAttacksGump(Mobile from, XmlCustomAttacks a) : base(0, 0)
			{
				...
				{
					var s = (SpecialAttack)m_attachment.Specials[i];
